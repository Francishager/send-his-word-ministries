        ...
        <div className="lg:col-span-3">
          ...
          <div className="mt-6 rounded-xl border bg-white p-4">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {pesapalEnabled && (
                <button onClick={payWithPesapal} className="rounded-md bg-emerald-600 px-4 py-2 text-white text-sm font-medium hover:bg-emerald-500">Pesapal</button>
              )}
              {mpesaEnabled && (
                <button onClick={payWithMpesa} className="rounded-md bg-green-600 px-4 py-2 text-white text-sm font-medium hover:bg-green-500">M-Pesa</button>
              )}
              {!!process.env.NEXT_PUBLIC_STRIPE_LINK && (
                <a href={process.env.NEXT_PUBLIC_STRIPE_LINK} target="_blank" rel="noopener" className="rounded-md bg-indigo-600 px-4 py-2 text-white text-sm font-medium hover:bg-indigo-500">Pay with Stripe</a>
              )}
              {!!process.env.NEXT_PUBLIC_PESAPAL_LINK && (
                <a href={process.env.NEXT_PUBLIC_PESAPAL_LINK} target="_blank" rel="noopener" className="rounded-md bg-emerald-600 px-4 py-2 text-white text-sm font-medium hover:bg-emerald-500">Pay with Pesapal</a>
              )}
              {!!process.env.NEXT_PUBLIC_MPESA_LINK && (
                <a href={process.env.NEXT_PUBLIC_MPESA_LINK} target="_blank" rel="noopener" className="rounded-md bg-green-600 px-4 py-2 text-white text-sm font-medium hover:bg-green-500">Pay with M-Pesa</a>
              )}
