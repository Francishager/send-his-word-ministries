...
const BookOpenIcon: React.FC<{ className?: string }> = ({ className }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 20l9-4V6l-9 4-9-4v10l9 4zM3 6l9 4 9-4"/></svg>
);
...
function BiblePage() {
  const navItems = [
    ...
    { name: 'Devotions', href: '/attendee/devotions', icon: BookOpenIcon, roles: [UserRole.ATTENDEE] },
  ...
  return (
    <PortalLayout userRole={UserRole.ATTENDEE} navItems={navItems} title="Attendee Portal">
      <div className="space-y-4">
        ...
        <p className="text-gray-600">Read Scripture and highlight/copy into your Notices.</p>
        <div className="rounded-md border p-4 bg-white">Coming soon: passage selector, highlight and copy tools.</div>
      </div>
    </PortalLayout>
  );
}

export default withAuth(BiblePage, [UserRole.ATTENDEE, UserRole.MINISTER, UserRole.ADMIN]);
