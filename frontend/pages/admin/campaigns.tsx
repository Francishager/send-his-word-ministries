...
const UsersIcon: React.FC<{ className?: string }> = ({ className }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor">
    <path
      ...
      strokeLinejoin="round"
      strokeWidth="2"
      d="M17 20h5v-2a4 4 0 0 0-3-3.87M9 20H4v-2a4 4 0 0 1 3-3.87M16 3.13a4 4 0 0 1 0 7.75M8 3.13a4 4 0 1 0 0 7.75M12 14a4 4 0 0 0-4 4h8a4 4 0 0 0-4-4z"
    />
  </svg>
);
const ChartIcon: React.FC<{ className?: string }> = ({ className }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor">
    <path
      ...
      strokeLinejoin="round"
      strokeWidth="2"
      d="M11 11V3m4 18V3m-8 10v8M3 21h18"
    />
  </svg>
);
...
function CampaignModal({
  ...
  onSave: (data: CampaignInput, id?: string) => Promise<void>;
}) {
  const [title, setTitle] = React.useState(initial?.title || '');
  const [description, setDescription] = React.useState(initial?.description || '');
  const [goal, setGoal] = React.useState<number>(
    initial?.goal_amount ? Number(initial.goal_amount) : 0
  );
...
function AdminCampaigns() {
  const navItems = [
    {
      ...
      href: '/admin/services',
      icon: CalendarIcon,
      roles: [UserRole.MINISTER, UserRole.ADMIN],
    },
    {
      name: 'Moderation',
  ...
  const onSave = async (data: CampaignInput, id?: string) => {
    if (id) await campaignsApi.update(id, data);
    ...
    <PortalLayout userRole={UserRole.ADMIN} navItems={navItems} title="Minister / Admin Portal">
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={onNew}
            className="inline-flex items-center rounded-md bg-indigo-600 px-4 py-2 text-white font-medium"
          >
            New Campaign
          </button>
        </div>
                ...
                <div key={c.id} className="p-4 flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-gray-600">
                      Status: {c.status} · Goal: ${Number(c.goal_amount).toLocaleString()}
                    </div>
                  </div>
                  <div className="space-x-2">
                    <button onClick={() => onEdit(c)} className="px-3 py-1 rounded border">
                      Edit
                    </button>
                    <button onClick={() => onDelete(c)} className="px-3 py-1 rounded border">
                      Delete
                    </button>
                  </div>
                </div>
